...
class Mamba2AttentionModel(Mamba2PreTrainedModel):
    def __init__(self, config: HybridMamba2Config):
        ...
        for idx in range(config.num_hidden_layers):
            if idx in config.attn_layer_idxs:
                tmp_layers.append(MHABlock(**config.attn_params, embed_dim=config.hidden_size, d_conv=config.conv_kernel, layer_idx=idx))
            else:
