...
class Tokenizer(nn.Module):
    ...
    def forward(self, x_num: Tensor) -> Tensor:
        x = self.weight[:-1] * x_num[:, :]

        x = torch.cat(
            [x, self.weight[-1].repeat(len(x), 1, 1)],
            dim=1,
        )
        bias = torch.cat(
            [
...
class FTTransformerSpace(nn.Module):
    def __init__(self, d_numerical, d_token=192, d_ffn_factor=4/3, n_heads=4, d_out=1):
        ...
        d_token = 192  # nn.ValueChoice([64, 128, 192, 256], label="d_token")
        # d_ffn_factor = nn.ValueChoice([4/3, 1/2, 3/4, 1], label="d_ffn_factor")
        d_hidden = d_token  # * d_ffn_factor
        n_layers = nn.ValueChoice(range(2, 6), label="n_layers")
        layer_dropout_rate = nn.ValueChoice(
            [0.0, 0.1, 0.2, 0.3, 0.4], label="transformer_dropout_rate")
        activation = nn.GELU()  # "__torch__.nni.retiarii.nn.pytorch.ReLU"  # F.gelu
