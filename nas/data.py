...
class MISO_Data_v1(Dataset):
    def __init__(self, lag_range, target):
        ...
        self.x = torch.tensor(
            self.preprocessed_df[self.features].values, dtype=torch.float32).reshape(-1, len(self.features))
        self.y = torch.tensor(
            self.preprocessed_df[self.target].values, dtype=torch.float32).reshape(-1, 1).to(device)
    ...
    def load_data(self, lag_range, target):
        ...
        rawdf = pd.read_csv(os.path.join(
            data_dir, 'MISO_220621-0627_1mnDataset_K96.csv'))

        self.features = ['pressure(hPa)', 'rh(%)', 'temp_sensor(C)', 'k96_lpl_raw',
                         'k96_spl_raw', 'k96_mpl_raw', "k96_h2o(ppm) ", "k96_ch4(ppm) "]
        if target == "ref_h2o(ppm)":
            self.features += ["k96_h2o(ppm) _" + str(i)
                              for i in range(1, lag_range)]
            extra_prefix = "k96_h2o(ppm) "
        elif target == "ref_ch4(ppm)":
            self.features += ["k96_ch4(ppm) _" + str(i)
                              for i in range(1, lag_range)]
        ...
        lag_df.columns = [extra_prefix + "_" +
                          str(j) for j in range(lag_range)]
...
class MISO_Data_v2(Dataset):
    def __init__(self, lag_range, target):
        ...
        self.load_data(lag_range, target)
        self.x = torch.tensor(
            self.preprocessed_df[self.features].values, dtype=torch.float32).reshape(-1, 1, len(self.features))
        self.y = torch.tensor(
            self.preprocessed_df[self.target].values, dtype=torch.float32).reshape(-1, 1).to(device)
        self.scaler = StandardScaler()
        self.x = self.scaler.fit_transform(self.x).to(device)
