...
from nni.nas.profiler.pytorch.utils import MutableShape, ShapeTensor, standardize_arguments, submodule_input_output_shapes, is_leaf_module, concat_name
...
def to_onnx(model: nn.Module, example_inputs: Any) -> Any:
    ...
    torch.onnx.export(model, example_inputs, "tmp.onnx", export_params=False)

    model = onnx.load("test.onnx", load_external_data=False)
...
class NnMeterProfiler(ExpressionProfiler):
    ...
    def __init__(self, model_space: ModelSpace, args: Any,
        ...
        if simplify_shapes:
            from nni.nas.profiler.pytorch.utils._expression import recursive_simplification
    ...
    def exhaust_combinations(self, name: str, module: nn.Module, shapes: dict[str, Any]) -> MutableExpression[float] | float:
        ...
        for counter, (condition, sample_module, inputs) in enumerate(combinations(module, input_shape)):
            ...
            try:
                latency = sum(self.predictor.predict(onnx_model, model_type='onnx').values())
            except:
