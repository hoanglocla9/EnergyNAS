...
def loading_customized_predictor(pred_info, mode="energy"):
    ...
    # load predictors
    predictors = {"latency":{}}
    ps = glob(os.path.join(latency_ppath, "**.pkl"))
    for p in ps:
        pname =  os.path.basename(p).replace(".pkl", "")
        with open(p, "rb") as f:
            logging.info("load latency predictor %s" % p)
            model = pickle.load(f)
    ...
    if mode == "energy":
        ...
        for p in ps:
            pname =  os.path.basename(p).replace(".pkl", "")
            with open(p, "rb") as f:
                logging.info("load power predictor %s" % p)
                model = pickle.load(f)
                predictors["power"][pname] = model
...
def load_config_file(fname: str, loader=None):
    ...
    try:
        ...
        logging.info(f"config file {filepath} not found, created")
        raise Exception ("Not exist predictors.yml file in current directory!!")
