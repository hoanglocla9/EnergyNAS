...
def compute_ampere(power_log_file, latency):
    ampere_list = []
    if not os.path.exists(power_log_file) or os.path.isdir(power_log_file):
        return 0
    expected_min_sample = latency * 30 / 5
    with open(power_log_file, "r") as f:
        ...
        for line in f:
            if line_idx >= 19:
                if len(line.strip()) == 0:
                    continue
                if len(line.split(";")[-1]) == 0: ## error
                    return 0 
                ampere = float(line.split(";")[0])
                if ampere < 0:
                    ampere = ampere * -1
                ampere_list.append(ampere)
            line_idx += 1

    if len(ampere_list) == 0 or len(ampere_list) < expected_min_sample:
        return 0

    top_ids = np.argsort(ampere_list)[-10:]
    top_values = [ampere_list[i] for i in top_ids]
...
def profile_models(backend, models, mode = 'ruletest', metrics = ["latency"], save_name = "profiled_results.json",
                   have_converted = False, log_frequency = 1, broken_point_mode = True, **kwargs):
    ...
    **kwargs: arguments for profiler, such as `taskset` and `close_xnnpack` in TFLite profiler
    ...
    for module in models.values():
        for id, model in module.items():
            if broken_point_mode and 'latency' in model and float(str(model['latency']).split("+-")[0]) != 0:
                # continue
                if float(str(model['latency']).split("+-")[0]) > 1 or 'power' in model: 
                    continue
                if float(str(model['latency']).split("+-")[0]) == -1: 
                    continue
                if "power_file" in model and model["power_file"] == "":
                    continue
                if 'power' in model:
                    continue 
                del model['power_file']
                
            #    del model['power']
            # if "HW" in model['config'] and (( 'CIN' in model['config'] and model['config']['HW'] ** 2 * (model['config']['CIN'])* 32 >= 350000000) or \
            #     ('COUT' in model['config'] and model['config']['HW'] ** 2 * (model['config']['CIN'] + model['config']['COUT'])* 32 >= 350000000)):
            #     continue
            if have_converted: # the models have been converted for the backend
                ...
                count += 1
                print("PROFILING: ", time.time() - start)
                # except Exception as e:
                #     open(error_save_path, 'a').write(f"{id}: {e}\n")
            time.sleep(1)
...
def convert_models(backend, models, mode = 'ruletest'):
    if isinstance(models, str):
        with open(models, 'r') as fp:
            models = json.load(fp)
    ...
    for key, module in models.items():
        ...
        for id, model in module.items():
            try:
                model_path = model['model']
                model['converted_model'] = backend.convert_model(model_path, model_save_path, model['shapes'])
                converted_models[key][id] = model
            except Exception as e:
                print(f"The model {model} is error")
...
def sample_and_profile_kernel_data(kernel_type, sample_num, backend, sampling_mode = 'prior', configs = None, mark = '', detail = True,
    ...
    if not os.path.exists(f"/media/locla/Data/tmp_data/workspace/predictor_build/results/{kernel_type}_prior.json"):
        models = generate_config_sample(kernel_type, sample_num, mark=mark, 
                                     sampling_mode=sampling_mode, configs=configs)
    else:
        with open(f"/media/locla/Data/tmp_data/workspace/predictor_build/results/{kernel_type}_prior.json") as f:
            models = json.load(f)  
