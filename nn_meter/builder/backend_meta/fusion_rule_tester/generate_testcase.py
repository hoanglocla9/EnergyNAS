...
class BasicFusion(BaseTestCase):
    ...
    def load_latency(self, testcase):
        if 'latency' not in testcase['block']: 
            self.latency['block'] = Latency("0")
        else:
        ...
        if op1_alias == op2_alias:
            op1_alias += '_1'
            op2_alias += '_2'
        if 'latency' not in testcase[op1_alias]: # or 
            self.latency[op1_alias] = Latency("0")
        else:
            self.latency[op1_alias] = Latency(testcase[op1_alias]['latency'])

        if 'latency' not in testcase[op2_alias]:
            self.latency[op2_alias] = Latency("0")
        else:
