...
class ConvAdd(TorchBlock):
    ...
    def get_model(self):
        class Model(nn.Module):
            def __init__(self, conv_op, add_op):
                super().__init__()
                self.add = add_op
                self.conv = conv_op
        ...
        return Model(self.conv_op, self.add_op)
...
class FCRelu(TorchBlock):
    def __init__(self, config, batch_size = 1):
        ...
        relu_op = Relu(fc_op.get_output_shape(), config)
        self.relu_op = relu_op.get_model()

    def get_model(self):
        return self.build_model([self.fc_op, self.relu_op])
...
class ConcatBlock(TorchBlock):
    def __init__(self, config, batch_size = 1):
        self.config = config
        # cins_list = [config['CIN1'], config['CIN2'], config['CIN3'], config['CIN4']]
        cins_list = []
        cin_keys = []
        for key in config:
            if "CIN" in key:
                cin_keys.append(key)
        for idx in range(1,len(cin_keys)+1):
            cins_list.append(config[f'CIN{idx}'])

        self.input_shape = [[cin, config["HW"], config["HW"]]  for cin in cins_list if cin != 0]
...
class ReduceMeanBlock(TorchBlock):
    def __init__(self, config, batch_size = 1):
        self.config = config
        self.input_shape = [config["HW"], config["HW"], config["CIN"]]
        self.input_tensor_shape = [self.input_shape]
        ...
        reducemean_op = GlobalAvgpool(self.input_shape, config)
        self.reducemean_op = reducemean_op#.get_model()
