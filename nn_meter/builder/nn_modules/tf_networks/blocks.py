...
class ConvAdd(TFBlock):
    ...
    def get_model(self):
        ...
        model = Model(self.conv_op, self.add_op)
        model(get_inputs_by_shapes(self.input_tensor_shape, self.batch_size))
        return model
...
class MaxPoolBNBlock(TFBlock):
    ...
    def get_model(self):
        return self.build_model([self.maxpool_op, self.bn_op])
...
class AvgPoolBNBlock(TFBlock):
    ...
    def get_model(self):
        return self.build_model([self.avgpool_op, self.bn_op])
    
class FCRelu(TFBlock):
    def __init__(self, config, batch_size = 1):
        self.config = config
