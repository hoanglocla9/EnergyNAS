...
def remove_cast_layers(graph):
    new_graph = {}
    inbounds, outbounds = [], []
    list_nodes = list(graph.values())
    first_cast_idx = 0
    for idx, (node_name, node) in enumerate(graph.items()):
        if not "type" in node["attr"]:
            if len(inbounds) == 0:
            ...
            if len(inbounds) != 0:
                ...
                previous_node_name = list(graph.keys())[first_cast_idx]
                new_graph[previous_node_name]["outbounds"] = [node_name]
            new_graph[node_name] = node
