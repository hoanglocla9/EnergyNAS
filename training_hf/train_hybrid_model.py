...
from nni.nas.nn.extras.llm_layers import HybridMamba2Config, HybridMamba2ForCausalLM
import os 
...
def get_dataset_pile(dataset_path="/root/llm_dataset/pile-uncopyrighted/", split="train", shuffle=True):
    if split == "train":
...
def get_dataset_alpaca(dataset_path="/home/picarib/Data/alpaca/", shuffle=True):
    dataset_path = dataset_path + "data/*.parquet]"
    dataset = load_dataset('parquet', data_files={'train':dataset_path}, split='train', streaming=True)
    if shuffle:
        return dataset.shuffle(buffer_size=10_000, seed=42)
    return dataset
...
model.to('cuda')
# train_dataset = get_dataset_pile(split="train", shuffle=False) 
# valid_dataset = get_dataset_pile(split="val", shuffle=False)
...
full_dataset = get_dataset_alpaca(shuffle=True) 
train_dataset = split_dataset_by_node(full_dataset, rank=int(os.environ["RANK"]), world_size=int(os.environ["WORLD_SIZE"]))
valid_dataset = split_dataset_by_node(full_dataset, rank=int(os.environ["RANK"]), world_size=int(os.environ["WORLD_SIZE"]))
